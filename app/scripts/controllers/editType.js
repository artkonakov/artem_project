...
angular.module('resdokWebApp')
    .controller('editTypeCtrl', ['$scope', 'ModalService', '$stamplay', '$route', '$routeParams', 'Notification', 'Upload', function ($scope, ModalService, $stamplay, $route, $routeParams, Notification, Upload) {
        ...
        $scope.updateType = function () {
            $stamplay.Object("types").patch($scope.type[0].id, $scope.type[0])
                .then(function (res) {
                    // success

                    new Notification({
                        message: 'Тип обновлен'
                    }, 'success');
                }, function (err) {
                    // error
                    new Notification({
